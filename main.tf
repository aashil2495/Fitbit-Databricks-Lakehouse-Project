...
# Create the managed Storage Container
...
# Create the unmanaged Storage Container
resource "azurerm_storage_container" "unmanaged" {
  name                  = "unmanaged"
  storage_account_name  = azurerm_storage_account.adls_gen2.name
  container_access_type = "private"
}
...
# Create a rawdatazone in the Unmanaged Container
resource "azurerm_storage_data_lake_gen2_path" "rawdatazone" {
  path               = "rawdatazone"
  filesystem_name    = azurerm_storage_data_lake_gen2_filesystem.rawdatazone.name
  storage_account_id = azurerm_storage_account.adls_gen2.id
  resource           = "directory"
}

# Create a checkpoint in the Unmanaged Container
resource "azurerm_storage_data_lake_gen2_path" "checkpoint" {
  path               = "checkpoint"
  filesystem_name    = azurerm_storage_data_lake_gen2_filesystem.checkpoint.name
  storage_account_id = azurerm_storage_account.adls_gen2.id
  resource           = "directory"
}
...
resource "databricks_metastore" "main" {
  ...
  storage_root         = "abfss://${azurerm_storage_account.adls_gen2.name}@${azurerm_storage_account.adls_gen2.name}.dfs.core.windows.net/meta"
  region               = "eastus"
  force_destroy        = true
}
